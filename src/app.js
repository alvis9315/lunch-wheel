(function () {
  ...
  const DEFAULT_FILTERS = {walk:'any',weather:'any',budget:'any',category:'all',diet:'any',rating:'any',from:'12:00',to:'13:00',open:true,noRepeat:false};
  ...
  const reviewUI=window.createLunchReviewUI({rpc,escape:e});
  ...
  function updateOriginView(){
    ...
    $('origin-note').textContent=originCustom?'／自行設定':defaultOriginVerified?'':'／基準點待校正';
  ...
  function applyOrigin(origin,custom=true){
    ...
    try{if(custom)localStorage.setItem(ORIGIN_STORAGE,JSON.stringify(state.origin));else localStorage.removeItem(ORIGIN_STORAGE);}catch{message('瀏覽器無法儲存出發點，這次設定僅在本頁有效。');}
  ...
  function openOrigin(position){
    ...
    $('origin-feedback').textContent='儲存在此瀏覽器，私人與共用名單都可使用。';
    if(!$('origin-dialog').open)$('origin-dialog').showModal();
  }
  function chooseOriginOnMap(){
    if(!mapView){$('origin-feedback').textContent='本機檔案預覽沒有互動底圖，請填入座標或使用目前位置。';return;}
  ...
  function locateOrigin(){
    if(!navigator.geolocation){$('origin-feedback').textContent='此瀏覽器不支援定位，請用地圖或填寫座標。';return;}
  ...
  function getLocal(mode=state.mode) {
    try {
      ...
      return mode==='local'&&old.mode==='demo'?{...old,localRecords:(old.localRecords||[]).filter(p=>!p.demo)}:{};
    } catch { return {}; }
  }
  function saveLocal() {
    if(!state.ready)return;
    try { localStorage.setItem(STORAGE+state.mode,JSON.stringify({selected:[...state.selected],lastId:state.lastId,localRecords:state.mode==='local'?state.records:undefined})); }
    catch { message('瀏覽器無法儲存資料，個人名單與勾選只在這次開啟有效。'); }
  }
  ...
  function renderAccess(){
    ...
    $('source-label').textContent=state.mode==='local'?'我的私人名單':active?'共用名單 · 管理者':'共用名單 · 瀏覽模式';
    $('mode-banner').textContent=state.mode==='local'?'個人名單只存在此瀏覽器；不會上傳到共用試算表。清除網站資料後會消失。':active?'管理者已登入，可以新增共用店家。登入最長 30 分鐘；網站不提供刪除功能。':'店家由管理者維護。大家都能抽選、分享品項心得與投票；新增店家需管理員登入。';
  }
  ...
  async function signOut(){
    ...
    if(token)try{await rpc('adminLogout',token);}catch{message('已清除此頁登入狀態；伺服器登出未完成，憑證最晚於原到期時間失效。');}
  }
  async function adminAction(){
    if(state.busy)return;
    if(adminSession){await signOut();return;}
    if(!adminConfigured){message('管理功能尚未設定，請擁有者設定 Apps Script 的 ADMIN_PASSPHRASE。');return;}
  ...
  async function loginAdmin(event){
    ...
    try{
      ...
      $('admin-dialog').close();renderAccess();message('登入成功，可以新增共用店家。');
    }catch(err){$('admin-feedback').textContent=err.message;}
    finally{passphrase='';button.disabled=false;}
  }
  function rpc(method,...args) {
    return new Promise((resolve,reject)=> {
      const timer=setTimeout(()=>reject(Error('連線逾時，請重新整理確認名單；新增可能已完成。')),30000);
      google.script.run.withSuccessHandler(value=>{clearTimeout(timer);resolve(value);}).withFailureHandler(err=>{clearTimeout(timer);reject(Error(err.message||'連接試算表失敗'));})[method](...args);
    });
  }
  function demoPlaces() {
    const day = C.weekday();
  ...
  function render() {
    ...
    $('lineup-cards').innerHTML=choices.map(p=>'<button class="mini-card" data-detail="'+e(p.id)+'"><span class="food-icon">'+(ICONS[p.category]||'🍽')+'</span><span><strong>'+e(p.name)+'</strong><small>'+e(p.category||'未分類')+' · '+e(Catalog.diets[p.diet]||Catalog.diets.unknown)+' · '+kmText(p)+'</small><small>'+(p.budget==null?'預算未標記':'NT$ '+p.budget)+'</small></span></button>').join('')||'<div class="empty-state">還沒有符合條件的選手。<br>到口袋名單勾選店家，或調整上方條件。</div>';
  ...
  function renderSaved() {
    ...
    $('selection-count').textContent='本次已勾選 '+state.selected.size+' / 20 間 · '+(state.mode==='local'?'保存在此瀏覽器':'共用名單由管理者維護');
    ...
    $('saved-list').innerHTML=list.map(p=>'<article class="saved-card '+(state.selected.has(p.id)?'selected':'')+'"><input type="checkbox" data-select="'+e(p.id)+'" aria-label="本次抽選 '+e(p.name||p.id)+'" '+(state.selected.has(p.id)?'checked':'')+' '+(state.busy?'disabled':'')+'><span class="food-icon">'+(ICONS[p.category]||'🍽')+'</span><button class="card-main" data-detail="'+e(p.id)+'"><strong>'+e(p.name||'店家資訊待載入')+'</strong><small>'+e(p.category||'未分類')+' · '+e(Catalog.diets[p.diet]||Catalog.diets.unknown)+' · '+kmText(p)+' · '+(p.budget==null?'預算未知':'NT$ '+p.budget)+'</small>'+statusHtml(p)+'</button></article>').join('')||'<div class="empty-state">'+(query?'沒有符合搜尋的店家。':'還沒有店家，從右側地圖找一間吧！')+'</div>';
  }
  function page(name) {
    if(state.busy||!state.ready) return;
  ...
  function resetFilters() {
    ...
    $('budget-filter').value='any';$('category-filter').value='all';$('lunch-from').value='12:00';$('lunch-to').value='13:00';$('open-filter').checked=true;$('repeat-filter').checked=false;render();loadWeather();
  }
  async function loadWeather() {
    ...
    try {
      ...
      const data=await response.json(), day=C.taipeiDay(), start=C.minutes(state.filters.from), end=C.minutes(state.filters.to);
      // Open-Meteo accumulated precipitation probabilities describe the preceding hour.
      const values=data.hourly.time.map((t,i)=>({t,v:data.hourly.precipitation_probability[i]})).filter(({t,v})=>t.startsWith(day)&&Number.isFinite(v)&&C.minutes(t.slice(11))>start&&C.minutes(t.slice(11))-60<end).map(x=>x.v);
      if(!values.length) throw Error();
      if(request!==weatherRequest)return;
      const probability=Math.max(...values);state.weather={rain:probability>=50,probability,day,from:state.filters.from,to:state.filters.to};
  ...
  function loadMaps(config={}) {
    if(!/^https?:$/.test(location.protocol)){
      $('map-placeholder').querySelector('p').textContent='本機檔案預覽使用示意地圖。部署到 Apps Script HTTPS 網址後會顯示免費互動地圖。';return;
    }
    if(!window.L){message('免費地圖未載入，可先用新增按鈕填寫店家。');return;}
  ...
  async function refreshCatalog() {
    ...
    try {
      ...
      state.selected=new Set([...state.selected].filter(id=>state.records.some(p=>p.id===id)));
      saveLocal();drawMarkers(state.records);message('已更新名單；營業資訊依自行維護的例行時段推估。');
  ...
  function showSearchResults(places) {
    ...
    $('search-results').innerHTML='<div class="panel-heading"><span class="field-note">'+(state.mode==='local'?'私人名單搜尋結果':'共用名單搜尋結果')+'</span><button class="text-button" id="close-search">收起</button></div>'+ (places.map(p=>'<button class="search-result" data-search-id="'+e(p.id)+'">'+e(p.name)+'<small>'+e(p.address)+'</small></button>').join('')||'<p class="field-note">名單還沒有這間店。可先到 Google Maps 查看，再按「新增店家」填入。</p>');
  }
  ...
  function renderDetail(p,existing) {
    ...
    let warning='';
    if(status.today===false) warning=(p.demo?'該店家今日沒營業。':'依填寫的例行時段，該店家今日沒營業；臨時調整請另行確認。')+(p.phone?'實際營業狀況請撥電話確認。':'');
    ...
    const mapLink=mapsUrl(p);
    $('detail-content').innerHTML='<p class="eyebrow">'+(p.demo?'DEMO · 虛構示範店家':'OUR SHARED FOOD LIST')+'</p><div class="detail-hero"><span class="food-icon">'+(ICONS[category]||'🍽')+'</span><div><h2 id="detail-title">'+e(p.name)+'</h2><p>直線 '+kmText(p)+'</p></div></div><p>'+e(p.address)+'</p>'+statusHtml(p)+'<p class="diet-detail">'+e(Catalog.diets[p.diet]||Catalog.diets.unknown)+(p.covered==='yes'&&!C.coveredFrom(p,state.origin)?' · 此出發點的避雨路線待確認':'')+'</p>'+(warning?'<div class="notice">'+e(warning)+(p.phone?'<br>'+phoneHtml(p):'')+'</div>':'')+(p.demo?'':'<p class="field-note">以上為自行填寫的例行時段，臨時營業異動請致電確認。</p>')+'<details><summary class="field-note">查看營業時間</summary><p>'+p.hoursText.map(e).join('<br>')+'</p></details>'+(mapLink?'<p><a target="_blank" rel="noopener" href="'+e(mapLink)+'">在 Google Maps 查看 ↗</a></p>':'')+
      (existing?'<div class="notice">已在目前名單。可用勾選框調整本次抽選，不會刪除店家。</div><button class="primary" id="detail-select">'+(state.selected.has(p.id)?'退出本次抽選':'加入本次抽選')+'</button>':'<form id="add-form"><div class="detail-form"><label>料理類型（自行標記）<select id="add-category">'+Object.keys(ICONS).map(c=>'<option'+(c===category?' selected':'')+'>'+c+'</option>').join('')+'</select></label><label>葷／素（自行確認）<select id="add-diet">'+Object.entries(Catalog.diets).map(([value,label])=>'<option value="'+value+'"'+(value===(p.diet||'unknown')?' selected':'')+'>'+e(label)+'</option>').join('')+'</select></label><label>每人預算 NT$（選填）<input id="add-budget" type="number" min="1" max="10000" step="1" placeholder="例如 180" value="'+(p.budget??'')+'"></label><label class="wide">從 '+e(state.origin.name||defaultOrigin.name)+' 出發，全程可避雨？<select id="add-covered"><option value="unknown">尚未確認</option><option value="yes"'+(p.covered==='yes'?' selected':'')+'>是，已確認全程有遮蔽</option><option value="no"'+(p.covered==='no'?' selected':'')+'>否，需要走露天路段</option></select></label></div><p class="field-note">'+(state.mode==='local'?'這些資料只儲存在此瀏覽器，不會上傳。':'這些資料會加入共用名單，供大家抽選。')+'營業中與否仍由抽選條件決定。</p><div class="detail-actions"><button type="submit" class="primary">'+(status.today===false?'已知今日休息，仍儲存店家':(state.mode==='local'?'加入我的口袋名單 ＋':'加入共用口袋名單 ＋'))+'</button></div></form>');
    if(existing&&state.mode==='live'){
  ...
  function previewManual(){
    ...
    try{
      ...
      const p=Catalog.hydrate({id:'local-'+crypto.randomUUID(),name:$('manual-name').value,address:$('manual-address').value,phone:$('manual-phone').value,mapsUrl:$('manual-url').value,location:{lat:Number($('manual-lat').value),lng:Number($('manual-lng').value)},category:'其他',budget:null,covered:'unknown',weeklyHours});
  ...
  async function addCurrent() {
    ...
    const submit=form.querySelector('button[type=submit]');submit.disabled=true;submit.textContent=state.mode==='local'?'正在存入此瀏覽器…':'正在存入共用名單…';
    state.busy=true;
    try {
      let result;
      if(state.mode==='live') result=await rpc('addCandidate',record,adminSession.token);
      else {const duplicate=state.records.find(r=>!r.demo&&Catalog.fingerprint(r)===Catalog.fingerprint(record));result={record:duplicate||record,duplicate:Boolean(duplicate)};}
      const saved=p.demo?{...p,...result.record}:Catalog.hydrate(result.record);
      if(!state.records.some(r=>r.id===saved.id))state.records.push(saved);
    ...
    } catch(err){if(err.message.includes('AUTH_REQUIRED'))clearAdmin();message(err.message);submit.disabled=false;submit.textContent='重試加入（會自動防止重複）';}
  ...
  function showResult(p,count) {
    ...
    $('result-content').innerHTML='<h3>'+e(p.name)+'</h3><p>'+e(p.category||'未分類')+' · '+(p.budget==null?'預算未標記':'NT$ '+p.budget)+' · 直線 '+kmText(p)+'</p><p>'+e(p.address)+'</p><p class="field-note">'+count+' 間店等機率抽選'+(p.demo?' · 虛構示範結果':' · 營業時段由名單維護，臨時異動請致電確認')+'</p><div class="result-actions"><button id="confirm-lunch" class="primary">今天就吃這家 ✓</button>'+(link?'<a class="secondary" target="_blank" rel="noopener" href="'+e(link)+'">步行導航 ↗</a>':'')+'</div>'+phoneHtml(p)+'<button id="spin-again" class="text-button">還想轉一次</button>';
    $('result-dialog').showModal();
  }
  async function enterSource(mode){
    if(state.busy)return;
    lockUI(true);state.ready=false;clearAdmin();$('source-status').textContent='正在準備名單…';
    try{
      let config={};
      if(mode==='live'){
        if(!window.google?.script?.run)throw Error('目前是本機預覽，尚未連接試算表。請完成 Apps Script 部署後使用共用名單；現在可選「自己建立」。');
        config=await rpc('getBootstrap');
        if(!config.sharedConfigured)throw Error('共用名單尚未設定，請擁有者先建立試算表並設定 SPREADSHEET_ID。');
        ...
        state.records=(await rpc('listCandidates')).map(r=>Catalog.hydrate(r));
      }else{
        const personal=getLocal('local');
        state.records=Array.isArray(personal.localRecords)?personal.localRecords.map(p=>Catalog.hydrate(p)):[];
      }
      const customOrigin=readOrigin();state.origin=customOrigin||{...defaultOrigin};originCustom=Boolean(customOrigin);
      state.mode=mode;state.filters={...DEFAULT_FILTERS};state.detail=null;resultPlace=null;
      const local=getLocal(mode);state.lastId=typeof local.lastId==='string'?local.lastId:null;
      const selected=Array.isArray(local.selected)?local.selected:state.records.slice(0,6).map(p=>p.id);
      state.selected=new Set(selected.filter(id=>state.records.some(p=>p.id===id)).slice(0,20));state.ready=true;
      ...
      if(!mapView)loadMaps(config);else {mapView.setView([state.origin.lat,state.origin.lng],16);originMarker.setLatLng([state.origin.lat,state.origin.lng]);}
      updateOriginView();drawMarkers(state.records);saveLocal();
    }catch(err){state.mode='loading';state.records=[];state.selected=new Set();$('source-status').textContent=err.message;}
    finally{lockUI(false);render();}
    if(state.ready){resetFilters();page(state.records.length?'wheel':'map');}
  }
  async function switchSource(){
    if(state.busy)return;saveLocal();lockUI(true);
    originPicking=false;$('origin-pick-panel').hidden=true;
    await signOut();weatherRequest++;state.ready=false;
    $('source-workspace').hidden=true;$('main-nav').hidden=true;$('source-page').hidden=false;
    $('source-status').textContent='自己建立的資料保留在此瀏覽器；清除網站資料後會消失。';
    lockUI(false);$('choose-local').focus();
  }
  function boot(){
  ...
  document.addEventListener('visibilitychange',()=>{if(!document.hidden&&state.ready&&!state.busy&&dateKey!==C.taipeiDay()){dateKey=C.taipeiDay();render();loadWeather();message('已跨日，將依新日期的例行時段重新篩選。');}});
  boot();
})();

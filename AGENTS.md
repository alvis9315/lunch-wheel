...
- v6.11 優先於下方舊規則：類別複選（OR，不限互斥），明確下一步後進挑店家；取消產品層級 20 間上限，仍至少 2 間。跨類別不重複、每店等機率；大型轉盤編號與完整名單對照，不可抽樣成前 20 間。返回更換類別保留篩選及仍適用勾選。Sites＋現有 Sheets 已選定但尚未實作；目前狀態以 docs/project-status.md 為準。
- 本機驗收優先：用 `npm run dev` 啟動可操作的本機環境，配合 `npm test`，畫面與流程先在本機通過再請使用者更新部署。可攜的開發服務放 scripts/local-*，個人 QA 留 .local-qa/；本機服務只用虛構資料與模擬 Google 帳號，不得納入正式部署產物。真正 OAuth／Sheets 權限仍須線上整合驗收，詳見 docs/local-testing.md。
- v6.10 優先：匿名／暱稱只在帳號設定一次，按「確認」儲存成功即關閉；評論沿用設定，僅提供更改入口，不重複填寫。改名須保留評論草稿與展開狀態。後端以 Members 設定寫入作者快照，若預覽名稱已過期則拒絕新寫入；既有評論及重試不隨改名更動。

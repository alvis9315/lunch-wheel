...
2. 若出現授權要求，使用擁有這份試算表的帳戶完成試算表讀寫授權。
...
4. 「誰可以存取」依使用方式選擇：若要同事不登入 Google 也能使用，選 **所有人**；若選「擁有 Google 帳戶的任何人」，訪客必須先登入 Google。選項依帳戶政策而異。
5. 按「部署」，複製「網頁應用程式」網址，結尾通常是 `/exec`。
6. 傳給同事的是這個 `/exec` 網址。`/dev` 是只有專案編輯者可用的測試網址。

可存取網站的人能讀取名單、留評論與投票；新增共用店家仍需網站內的管理密語。網站的存取設定與試算表共用設定是兩件事。

官方部署與身分說明：[Apps Script Web Apps](https://developers.google.com/apps-script/guides/web)。

## 6. 上線檢查

1. 開啟 `/exec`，選「使用現有資料」，確認店家名單出現。
2. 勾選至少 2 間，先關閉不適用的篩選，確認轉盤約 2.5 秒後顯示結果。
3. 更換出發點，確認距離更新；測試葷食／素食篩選。
4. 用一則真實用餐經驗測試評論及讚／爛，回到試算表確認 `Reviews`、`ReviewVotes` 有資料。
5. 用無痕視窗確認未登入管理員也能抽選、評論，但不能新增店家。
6. 用 iPhone Safari 開啟正式網址，確認地圖、對話框、撥號與導航。若定位被拒絕，仍可用地圖選點或輸入座標。

「自己建立」只存各自瀏覽器，不會寫進共用試算表。共用名單空白時，請先完成資料匯入或登入管理員新增店家。

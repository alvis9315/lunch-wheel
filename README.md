...
- **現有名單**：讀取私人 Google 試算表中的店家。一般訪客可勾選、篩選、抽選，並登入 Google 帳戶後留品項評論與讚／爛；新增店家仍限管理者通過後端登入。資料修改由擁有者在試算表維護。
- 兩個入口的 localStorage 分開，共用模式只存勾選與上次結果，不把共用店家複製到私人名單。原 v2 本機真實新增資料會遷移，虛構示範資料不帶入。
- **儲存庫不含 Google 帳戶設定**。本機檔案能使用私人模式；共用模式需完成以下部署，記名評論另依 v6 登入指南設定。
...
評論保存經 Google 驗證的信箱、名稱與暱稱，以及建立時間和重試識別碼。公開評論顯示暱稱或 Google 名稱，不回傳完整信箱或帳號識別；同一 Google 帳號換裝置仍共用投票狀態。舊匿名評論及投票保留，無法補造帳號關聯。一人擁有多個帳號仍可分別投票。
...
- `npm test` 執行語法檢查、建置與 tests/ 中的四組測試，無額外測試套件依賴。

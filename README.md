...
**目前版本 6.11.0：**餐點類別可複選，按「下一步」才挑店家；取消抽選 20 間上限，至少選 2 間。大量名單用編號搭配完整店名對照，每間機會相同。更新 ZIP 的 **Code.gs、Index.html**，儲存後編輯原部署並選「新版本」。試算表欄位及登入設定不用改。

**[專案狀態與待辦](docs/project-status.md)**：本次更新步驟、目前功能、驗證範圍、已知限制，以及已選定但尚未實作的「Sites 前端＋現有試算表」方向。舊版紀錄中的 20 間限制與單選流程已由 6.11 取代。

部署 ZIP 僅含可貼入 Apps Script 的成品與文件；完整原始碼及本機測試工具請從此 GitHub 儲存庫下載。

**先在本機試用：**在專案資料夾執行 `npm run dev`，開啟 http://127.0.0.1:5173/；Windows 也可雙擊 `start-local.cmd`。可用範例店家與兩個測試帳號試轉盤、評論、暱稱和投票，不會寫入線上試算表。修改程式後重新整理即可，詳見 [本機測試說明](docs/local-testing.md)。

**6.10.0：**匿名／暱稱統一在帳號設定，按「確認」成功後自動關閉視窗；評論沿用名稱、不再重填，改名保留正在寫的內容。已送出評論保留原名稱。更新 ZIP 內 **Code.gs、Index.html**，再編輯原部署並選「新版本」；Google 登入與試算表設定保留。詳見 [留言名稱與隱私](docs/review-privacy.md)。

**6.8.0：**店家名單改用完整店家頁閱讀資訊與評論；新增全選目前顯示、評論送出前確認、共用載入動畫與團長 Google 登入設定檢查。推薦店家區塊移到底部，移除重複提示。評論先只新增，不提供修改或刪除。更新 **Code.gs、Index.html** 並部署新版本；登入設定仍需在私人專案完成。[更新與登入排查步驟](docs/browsing-and-login.md)。
...
| 抽選 | 個人勾選至少 2 間、不設 20 間上限；套用 AND 篩選後至少 2 間；等機率、2.5 秒動畫 |

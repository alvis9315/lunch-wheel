# 午餐俱樂部 · 免費版（v6.6：推薦店家與試算表分類同步）

**6.6.0：**入口改讀每間店的實際分類，支援你提供的 20 類（包含「台式」），新分類不必改程式。新增「我要推薦店家」：店名與 Google Maps 連結必填，Google 登入後投稿，團長另行確認再加入名單；投稿自動存進 Suggestions 分頁。同步整理一般使用者提示與團長用語。更新 **Code.gs、Index.html** 並部署新版本。詳見 [投稿、分類與更新說明](docs/suggestions-and-categories.md)。

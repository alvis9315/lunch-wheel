# 午餐俱樂部 · 免費版（v6.9：餐點排列與固定配色）

**6.9.0：**選餐點頁新增每排 4／8 個切換並記住設定；窄螢幕與手機自動調整。類別入口、挑店家、店家名單共用固定七組配色，多類別店家顯示分色背景與類別標籤。分數形容詞顯示在留言表單及已送出的評論；查看 [排列、配色與分數說明](docs/layout-and-colors.md)。更新 ZIP 內 **Code.gs、Index.html**，再編輯原部署並選「新版本」；Google 登入與試算表設定保留。

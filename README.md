# 午餐俱樂部 · 免費版（v5：葷素篩選與自訂出發點）

本版已移除 Google Maps JavaScript／Places API，不需要 Maps 金鑰、綁卡或啟用 Google Cloud 計費。使用現成免費／開源資源，依各服務允許的用量使用，不會自動升級成付費。

## 兩個入口

- **自己建立**：從空白名單開始，自由新增；店家、勾選與上次結果只存在 localStorage，不呼叫試算表接口、不上傳店家。清除瀏覽器網站資料、換瀏覽器或裝置後無法取得原名單。地圖與天氣仍需網路。
- **使用現有資料**：讀取私人 Google 試算表中的店家。一般訪客可勾選、篩選、抽選，並匿名留品項評論與讚／爛；新增店家仍限管理者通過後端登入。資料修改由擁有者在試算表維護。
- 兩個入口的 localStorage 分開，共用模式只存勾選與上次結果，不把共用店家複製到私人名單。原 v2 本機真實新增資料會遷移，虛構示範資料不帶入。
- **尚未連接任何實際 Google 帳戶／試算表**。本機檔案能使用私人模式；共用模式需完成以下部署。
...
| 共用資料庫 | Google 試算表 + Apps Script 原生 SpreadsheetApp，無外部 HTTP 權限 |
| 品項評論 | 共用店家詳情可填品項、回饋、-100～200 整數分數；滑桿與直接輸入連動，即時顯示分級文案 |
| 評論投票 | 同一瀏覽器對每則評論可選讚／爛，互斥、可改票與取消；不影響原分數或抽選機率 |
| 名單 | 私人可自行新增；共用需管理員登入才可新增；網站不提供刪除；同名及約相同座標去重；總名單可超過 20 間 |
...
營業篩選明確標為「依填寫的例行時段推估」。未知日不當作營業或休息。每天可填 1～2 段，支援午休間斷與跨夜；關門早於開門表示翌日，不支援起訖相同代表 24 小時。臨時停業／特殊假日請電話確認。有電話會提供撥號；沒有電話不編造資料。
...
用瀏覽器開啟 `dist/Index.html`，選「自己建立」開始新增。直接 file:// 開啟沒有有效 HTTP Referer，因此不向 OSM 公共服務請求圖磚，使用示意地圖；可按按鈕自行填座標。部署到 Apps Script HTTPS 網址後使用互動底圖。
...
5. 「部署 → 新增部署 → 網頁應用程式」。以擁有者執行，存取範圍按實際使用者設定。能開網站的人可讀取名單、寫品項評論與投票；新增店家一定檢查後端管理員憑證。
6. 開啟 `/exec`，選「使用現有資料」→「管理員登入」，輸入你設定的密語，再透過地圖／新增按鈕輸入店家。用第二個瀏覽器確認可讀取、抽選，且未登入不能新增。iPhone Safari 登入、地圖拖移、撥號、導航需實機驗收。
...
- 公開資料／登入接口為 `getBootstrap`、`listCandidates`、`adminLogin`、`adminLogout`、`addCandidate`。v4 增加 `listReviews`、`addReview`、`setReviewVote`；它們不會授予新增店家的權限。驗證等內部函式以 `_` 結尾，不能由 google.script.run 呼叫。無店家 delete/update 接口；投票可以改票或取消。
...
同一份試算表拆為三個分頁，程式自動建立所需表頭：

| 分頁 | 一列代表 | 關聯 |
|---|---|---|
| RestaurantsFree | 一間店家 | id |
| Reviews | 一則品項／回饋／分數 | restaurant_id 對應店家 id |
| ReviewVotes | 一個瀏覽器對一則評論的投票狀態 | review_id 對應評論 id；同組合改票更新同列 |

評論另存建立時間、重試識別碼與作者瀏覽器識別碼的雜湊；投票另存瀏覽器識別碼的雜湊與更新時間。後端不回傳這些識別欄位。免登入識別只能記住同一瀏覽器，清除網站資料或換裝置會被視為新訪客，不能保證一個真人只有一票。
...
共用新增先經伺服器驗證登入權限與欄位，Script Lock 包住授權檢查、去重及寫入；試算表文字以安全字串保存，避免名稱被當作公式。無 Google API 金鑰讀取、無 UrlFetchApp。個人勾選與上次用餐記錄仍存在瀏覽器，與他人分開。
...
- `npm test` 執行語法檢查、建置與 tests/ 中的三組測試，無額外測試套件依賴。
...
- 本機 Chromium 額外通過兩入口、管理員、評論／投票、定位／地圖選點、出發點持久化、天氣與導航更新、六種寬度測試；截圖已檢視。這些瀏覽器測試工具與影像保留於本機，不包含於儲存庫。

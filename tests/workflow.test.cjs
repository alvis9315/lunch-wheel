...
originals[3][4]='<script>';assert.equal(f.box.listCandidates()[2].category,'其他');
...
assert.equal(Catalog.normalizeCategory(' 冰店 '),'冰店');assert.equal(Catalog.read({category:'冰店'}).category,'冰店');
